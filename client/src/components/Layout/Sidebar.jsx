...
export default function Sidebar({ open, onClose }) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 w-56 shrink-0 border-r border-border bg-card/95 p-4 shadow-sm transition-transform duration-200 md:static md:translate-x-0 ${
      ...
      >
        <nav className="space-y-1">
          ...
          {isAdmin && (
            <>
              <div className="pt-3 pb-1 text-xs font-semibold uppercase tracking-wide text-muted opacity-70">
                Admin
              </div>
              {adminItems.map((item) => (
                <NavItem key={item.to} {...item} onNavigate={onClose} />
